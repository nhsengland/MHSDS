...
SELECT  
       ...
       m.Measure_Value,
	   s.[Standard],
	   CASE WHEN p.[Plan] > 1 OR p.[Plan] = 0 THEN CAST(p.Plan_STR AS float) ELSE p.[Plan] END AS [Plan], -- temp fix for issue with planning reference data 
	   m.Measure_Value/ NULLIF(CASE WHEN p.[Plan] > 1 OR p.[Plan] = 0 THEN CAST(p.Plan_STR AS float) ELSE p.[Plan] END,0) as Plan_Percentage_Achieved,
	   s.Standard_STR,
	   p.Plan_STR
...
and m.Measure_Type = p.Measure_Type
...
SELECT  
	   ...
	   m.Plan_STR,
	   CAST( FORMAT(m.Plan_Percentage_Achieved,'P1') as varchar) as Plan_Percentage_Achieved_STR,
