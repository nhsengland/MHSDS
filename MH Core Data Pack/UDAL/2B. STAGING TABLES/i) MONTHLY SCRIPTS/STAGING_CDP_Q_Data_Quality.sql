...
AND ([Breakdown category] IN ('Performance','Non-Submitter')) --CHANGED HERE
...
WHERE 	
Dashboard = 'Submission status - time series'
AND ([Breakdown category] IN ('Performance', 'Non-Submitter')) --LS AMENDED - TEST
AND [Region code] <> 'Wales'	
AND [ReportingPeriodEndDate] BETWEEN @RPStart AND @RPEnd
AND Der_OrgSubmissionStatus <> 'No longer in scope'
AND Org_Type = 'Provider'
...
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
 ...
 --select * from [MHDInternal].[STAGING_CDP_Q_Data_Quality] where measure_type = 'denominator'
