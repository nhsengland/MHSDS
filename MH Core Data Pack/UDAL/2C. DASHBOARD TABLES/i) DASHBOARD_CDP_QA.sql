...
SELECT *
  ...
  INTO [MHDInternal].[DASHBOARD_CDP_QA]
...
SELECT *
  ...
  FROM [MHDInternal].[STAGING_CDP_Dementia_Historic]
...
SELECT  *
  FROM [MHDInternal].[STAGING_CDP_S_PH_SMI]

UNION

SELECT *
  FROM [MHDInternal].[STAGING_CDP_F_NHS_Talking_Therapies_Quarterly]
  WHERE CDP_Measure_ID NOT IN ('CDP_F01','CDP_F02','CDP_F03')

UNION 

SELECT *
  FROM [MHDInternal].[STAGING_CDP_O_OAPs]
 WHERE CDP_Measure_ID NOT IN ('CDP_O01')

UNION 

--SELECT * 
--  FROM [MHDInternal].[STAGING_CDP_B_NHS_Talking_Therapies_Monthly_Historic]
--  --WHERE CDP_Measure_ID NOT IN ('CDP_B04','CDP_B06','CDP_B07')

--  UNION

SELECT *
  FROM [MHDInternal].[STAGING_CDP_B_NHS_Talking_Therapies_Monthly]
  --WHERE CDP_Measure_ID NOT IN ('CDP_B06','CDP_B07')
...
SELECT *
  FROM [MHDInternal].[STAGING_CDP_C_CYP_ED_Historic]

UNION 

SELECT *
  FROM [MHDInternal].[STAGING_CDP_E_EIP]

UNION 

SELECT *
  FROM [MHDInternal].[STAGING_CDP_P_Perinatal]
  WHERE CDP_Measure_ID NOT IN ('CDP_P01')
...
FROM  [MHDInternal].[REFERENCE_CDP_METADATA] meta
  
LEFT JOIN [MHDInternal].[DASHBOARD_CDP] old
       ON meta.CDP_Measure_ID = old.CDP_Measure_ID

LEFT JOIN [MHDInternal].[DASHBOARD_CDP_QA] new 
...
SELECT old.Reporting_Period, 
  ...
  FROM [MHDInternal].[DASHBOARD_CDP] old
