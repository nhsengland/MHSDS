...
SELECT
	...
	SUM(CASE WHEN a.Der_DirectContactOrder IS NOT NULL AND a.ConsMediumUsed IN ('01', '02', '03', '04') THEN 1 ELSE 0 END) AS TotalContacts, -- Calculate total contacts per referral per month
	...
	SUM(CASE WHEN a.Der_DirectContactOrder IS NOT NULL AND a.ConsMediumUsed IN ('02', '04') THEN 1 ELSE 0 END) AS TotalContactsTelephone, -- Calculate total contacts per referral per month that were via telephone
...
SELECT
	...
	SUM(CASE WHEN a.Der_SNoMEDProcCode IS NOT NULL AND a.ConsMediumUsed IN ('01', '02', '03', '04') THEN 1 ELSE 0 END) AS TotalContacts, -- Calculate total contacts per referral per month (when a SNoMed code is used this is a contact)
...
SUM(CASE WHEN r.EmployStatusLast = '01' THEN 1 ELSE 0 END) AS AllEmpDenom
INTO #Agg
FROM #Master r
...
ORDER BY Person_ID, UniqMonthID
...
SELECT  
	...
	a.AllDemom, -- Creates 2 denominators that are needed to calculate proportions in Tableau
	a.AllEmpDenom,
	a.Caseload AS CaseloadDenom, -- Create duplicates for 6 measures that are needed as denominators to calculate proportions in Tableau
	a.TotalAccessed AS TotalAccessedDenom,
...
SELECT 
	...
	CASE WHEN MeasureName IN ('SourceCatMissing', 'SourceCatNotMissing', 'AgeCatMissing', 'AgeCatNotMissing', 'EthnicityCatMissing', 'EthnicityCatNotMissing', 'GenderCatMissing', 'GenderCatNotMissing', 'DeprivationQuintileMissing', 'DeprivationQuintileNotMissing', 'EmployStatusMissing', 'EmployStatusNotMissing') THEN a.AllDemom
		...
		WHEN MeasureName = 'CaseloadAccessed' THEN CaseloadDenom
		WHEN MeasureName IN ('SeenIn7', 'SeenIn8To30', 'SeenInOver30') THEN TotalAccessedDenom
		WHEN MeasureName IN ('TotalContactsF2F', 'TotalContactsTelephone', 'TotalContactsVideo', 'TotalContactsOtherMedium') THEN a.TotalContactsDenom
		WHEN MeasureName IN ('EmployedAtDischarge', 'NotEmployedAtDischarge', 'UnknownEmployedAtDischarge', 'PrevTimeInCaseloadSinceReferredOver180', 'PrevTimeInCaseloadSinceReferred91To180', 'PrevTimeInCaseloadSinceReferred0To90', 'PrevTimeInCaseloadSinceAccessedOver180', 'PrevTimeInCaseloadSinceAccessed91To180', 'PrevTimeInCaseloadSinceAccessed0To90') THEN a.ClosedReferralsDenom
