...
WHERE e.Der_Use_Submission_Flag = 'Y' AND e.UniqMonthID BETWEEN @STARTRP AND @ENDRP-1 --to ignore primary data already in pre pub table
...
-- ADD IN DATA FROM TEWV WHERE IPS TEAMS ARE EMBEDDED WITHIN EIP TEAMS AND VIRTUAL TEAMS WOULD NOT WORK
...
SELECT
	...
	map.Region_Code,
	map.Region_Name,
	CASE WHEN a.RN = 1 THEN r.ReportingPeriodStartDate END AS ReferralRequestReceivedDate,
	NULL AS ServDischDate,
	r.AgeServReferRecDate,
	r.Gender,
	r.SourceOfReferralMH,
	r.EthnicCategory,
	e.EmployStatus,
	e.WeekHoursWorked,
	CASE WHEN a.RN = 1 THEN 1 ELSE 0 END AS AccessFlag,
	a.Contacts
...
FROM [NHSE_MHSDS].[dbo].[MHS004EmpStatus] e
WHERE e.Der_Use_Submission_Flag = 'Y' AND e.OrgIDProv = 'RX3' AND e.UniqMonthID BETWEEN @STARTRP AND @ENDRP-1 --to ignore primary data already in pre pub table
...
FROM [NHSE_MH_PrePublication].[dbo].[V4_MHS004EmpStatus] e
WHERE e.Der_Use_Submission_Flag = 'Y' AND e.OrgIDProv = 'RX3' AND e.UniqMonthID = @ENDRP) e ON r.RecordNumber = e.RecordNumber
...
(SELECT
	a.RecordNumber,
	a.UniqServReqID,
	SUM(CASE WHEN a.[Der_SNoMEDProcCode] IS NOT NULL THEN 1 ELSE 0 END) AS Contacts,
	ROW_NUMBER () OVER(PARTITION BY a.UniqServReqID ORDER BY a.RecordNumber ASC) AS RN

FROM [NHSE_Sandbox_MentalHealth].dbo.PreProc_Interventions a

WHERE a.OrgIDProv = 'RX3' AND a.Der_SNoMEDProcCode IN ('1082621000000104', '772822000')
...
WHERE r.ReferralRequestReceivedDate >= '2016-01-01' AND r.UniqMonthID BETWEEN @STARTRP AND @ENDRP AND a.Contacts IS NOT NULL
